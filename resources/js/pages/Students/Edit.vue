...
import { Camera, Upload, ArrowLeft, RefreshCw, Edit3, Sparkles, CheckCircle2, ShieldCheck } from '@lucide/vue';
...
<template>
    <AppLayout :breadcrumbs="[{ title: 'Data Siswa', href: '/students' }, { title: 'Edit Siswa', href: `/students/${student.id}/edit` }]">
        ...
        <div class="p-6 max-w-4xl mx-auto space-y-6">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                        <Edit3 class="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                        Edit Data Siswa & Wajah InsightFace
                    </h1>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        Perbarui informasi NISN, Nama, Kelas, atau perbarui foto referensi wajah siswa.
                    </p>
                </div>
                ...
                >
                    <ArrowLeft class="w-4 h-4" /> Batal & Kembali
                </Link>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
                ...
                <div class="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                                <Sparkles class="w-4 h-4 text-indigo-500" /> Foto Wajah Terdaftar InsightFace
                            </h3>
                            <p class="text-xs text-slate-500 dark:text-slate-400">
                                Biarkan kosong jika tidak ingin mengganti foto referensi wajah yang sudah ada.
                            </p>
                        </div>
                        ...
                        <div class="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl text-xs font-semibold">
                            <button
                                type="button"
                                @click="captureMode = 'none'; stopCamera(); resetPhoto();"
                                :class="['px-3 py-1.5 rounded-lg transition cursor-pointer', captureMode === 'none' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500']"
                            >
                    ...
                    <div v-if="captureMode === 'none'" class="flex items-center gap-4 bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
                        <div class="w-20 h-20 rounded-2xl bg-slate-200 dark:bg-slate-800 overflow-hidden border border-slate-300 dark:border-slate-700 shadow-sm">
                            <img v-if="student.photo_url" :src="student.photo_url" alt="Foto Siswa" class="w-full h-full object-cover" />
                            <div v-else class="w-full h-full flex items-center justify-center text-slate-400 text-xs font-bold">Belum Ada</div>
                        </div>
                        <div>
                            <span v-if="student.has_face_registered" class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 rounded-full border border-emerald-200 dark:border-emerald-500/20">
                                <ShieldCheck class="w-3.5 h-3.5" /> Model AI Wajah Terdaftar (512 Dimensi)
                            </span>
                            <span v-else class="text-xs text-amber-500 font-semibold">Foto Wajah Belum Terdaftar</span>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                Pilih opsi di atas untuk mengganti foto referensi jika wajah siswa mengalami perubahan.
                            </p>
                        </div>
                    </div>
                    ...
                    <div v-if="captureMode === 'webcam'" class="space-y-4">
                        <div class="relative max-w-md mx-auto aspect-video bg-slate-950 rounded-2xl overflow-hidden border border-slate-800 flex items-center justify-center">
                            ...
                            <img v-if="previewImage" :src="previewImage" alt="Preview Baru" class="w-full h-full object-cover" />
                        ...
                        <div class="flex justify-center gap-3">
                            ...
                            >
                                <Camera class="w-4 h-4" /> Ambil Foto Baru Siswa
                            </button>
                    ...
                    <!-- Upload File Box -->
                    <div v-else-if="captureMode === 'file'" class="space-y-4 max-w-md mx-auto">
                        <div class="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-6 text-center hover:border-indigo-500 transition">
                            ...
                            <div v-else class="space-y-2">
                                ...
                                <p class="text-xs text-slate-500">Pilih berkas foto JPG/PNG baru</p>
                            </div>
                    ...
                    <div v-if="previewImage" class="flex items-center justify-center gap-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                        <CheckCircle2 class="w-4 h-4" /> Foto Baru Siap Diekstrak InsightFace AI
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200 dark:border-slate-800 pt-6 flex justify-end">
                    <button
                        ...
                        class="px-8 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 font-bold text-sm text-white shadow-lg shadow-indigo-500/20 disabled:opacity-50 transition cursor-pointer"
                    >
                        {{ form.processing ? 'Menyimpan Perubahan...' : 'Simpan Perubahan Data Siswa' }}
                    </button>
                </div>
            </form>
        </div>
